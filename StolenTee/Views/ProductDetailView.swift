...
struct ProductDetailView: View {
    ...
    init(product: Product) {
        ...
        _viewModel = StateObject(wrappedValue: CustomizerViewModel(product: product))
    }
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $showingCustomizer) {
            CustomizerView(viewModel: viewModel)
        }
    }
    ...
    private var quantityPicker: some View {
        QuantityPicker(quantity: $viewModel.quantity)
    }
        ...
        VStack(spacing: Theme.Spacing.sm) {
            ...
            if viewModel.quantity > 1 {
                ...
                HStack {
                    ...
                    if let basePrice = product.variants.first?.basePrice {
                        Text("$\(String(format: "%.2f", basePrice * Double(viewModel.quantity)))")
                            .font(Theme.Typography.titleLarge)
                            .fontWeight(.bold)
                            .foregroundColor(Theme.Colors.text)
                    }
    ...
    private var ctaButton: some View {
        ...
        .primaryButtonStyle(isEnabled: !viewModel.selectedColor.isEmpty && !viewModel.selectedSize.isEmpty)
        .disabled(viewModel.selectedColor.isEmpty || viewModel.selectedSize.isEmpty)
    }

    // MARK: - Helpers

    private var availableColors: [String] {
        Array(Set(product.variants.map { $0.color })).sorted()
    }

    private var availableSizes: [String] {
        Array(Set(product.variants.map { $0.size })).sorted()
    }
}
