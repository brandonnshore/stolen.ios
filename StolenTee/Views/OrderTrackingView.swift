    ...
    private func trackingTimeline(_ order: Order) -> some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                TrackingStep(
                    title: "Processing",
                    date: order.status == .processing || order.status == .shipped || order.status == .delivered ? Date() : nil,
                    isCompleted: order.status == .processing || order.status == .shipped || order.status == .delivered,
                    isLast: false
                )

                TrackingStep(
                    title: "Shipped",
                    date: order.status == .shipped || order.status == .delivered ? Date() : nil,
                ...
                TrackingStep(
                    title: "Delivered",
                    date: order.status == .delivered ? Date() : nil,
